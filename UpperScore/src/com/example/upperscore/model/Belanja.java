...
public class Belanja
{
	...
	public Belanja (String sm) //ctorparam
	{
            supermarket=sm;
            shoppingList = new ArrayList<Item>();
	}
	...
	// Getters and Setters
	public int getLocation(String barcode) {
		int index = 0;
		for (Item i : shoppingList) {
			if (i.getBarcode().getId().equals(barcode)) {
				break;
			} else {
				++index;
			}
        ...
        //id pasti ada di supermarket, kalo id udah ada di shoppinglist ditambah quantitynya aja
	{
        ...
        if(!found)
        {
        	DatabaseHandler handler = new DatabaseHandler(Source.getInstance().getContext());
            Produk produk = handler.getProduk(supermarket, id.getId());
            handler.close();
